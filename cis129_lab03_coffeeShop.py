...
"""Assigns transaction status as 'y' so it can execute on the first run.
At the end of the transaction, it will ask if you want to begin a new transaction.
If 'y' is entered, the script will restart, if 'n' is entered, the script will end"""
...
while NewTransaction.lower() == 'y':   
    ...
    # Prints a receipt if an order was placed.
    if Subtotal > 0:
        print()
        print('***************************************')
        ...
        print('***************************************')
        print()
        print("Thank You for Visiting Seb's Fantastic Cafe!")
        print('Please visit again soon!')
        print()
        while True:
            # Asks the user to continue with a new transaction or not. 
            # Ensures value is only a 'y' or a 'n'.
            NewTransaction = input('New Transaction?(y/n) ')
            if NewTransaction.lower() == 'y' or NewTransaction.lower() == 'n':
                break
            if NewTransaction.lower() != 'y' and NewTransaction.lower() != 'n':
                print("Please only answer with the letter 'y' or 'n'.")
        print()
        
    # Alternate message if customer does not order anything.
    if Subtotal == 0:
        print()
